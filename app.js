...
function bloqueTurno(grupoTurnos) {
  ...
  const clicAdmin = SESION.rol === 'admin' && grupoTurnos.length === 1
    ...
    : SESION.rol === 'admin'
      ...
      : '';
  const miTurno = grupoTurnos.find(t => agentesDeTurno(t.agente).includes(SESION.usuario));
  const clic = clicAdmin || (miTurno ? `abrirModalCambio('${miTurno.fecha}')` : '');
    ...
    return `
      <div class="card ${claseTarjeta}">
        ...
        ${e.estado === 'Hecho' && e.agentesFirmantes && e.agentesFirmantes.length
          ? `<p style="font-size:12px;color:#374151;">Firmado por: ${e.agentesFirmantes.map(nombreAgente).join(' y ')}</p>`
