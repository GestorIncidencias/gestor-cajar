...
async function cargarTurnos() {
  ...
  const llamadas = [
    ...
    llamar('getIntereses', { mes: mesISO(mesActual) })
  ];
  if (SESION.rol === 'admin') llamadas.push(llamar('getIndisponibilidades', { mes: mesISO(mesActual) }));
  const [rTurnos, rCambios, rIntereses, rIndisp] = await Promise.all(llamadas);
...
async function cargarDisponibilidad() {
  document.getElementById('mesLabel').textContent = mesActual.toLocaleDateString('es-ES', { month: 'long', year: 'numeric' });
  const [rTurnos, rIndisp] = await Promise.all([
    llamar('getTurnos', { mes: mesISO(mesActual) }),
    llamar('getIndisponibilidades', { mes: mesISO(mesActual) })
  ]);
