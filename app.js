...
async function cargarTurnos() {
  ...
  const [rTurnos, rCambios, rIntereses] = await Promise.all([
...
function abrirModalAsignarTurno(id, fecha, horaInicio, horaFin, agenteCampo) {
  ...
  document.getElementById('asigInteresados').textContent = interesados.length
    ? 'Se han apuntado: ' + interesados.map(u => u + ' · ' + nombreAgente(u)).join(', ')
    : 'Nadie se ha apuntado todavía a este turno.';
