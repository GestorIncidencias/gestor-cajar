...
async function guardarDocumentoAgregado() {
  ...
  await conProcesando(boton, 'Guardando…', async () => {
    ...
    let documentoUrl = document.getElementById('agregarDocUrl').value.trim();
    const archivo = document.getElementById('agregarDocArchivo').files[0];
    if (archivo) {
      boton.innerHTML = '<span class="spinner"></span>Subiendo…';
      const contenidoBase64 = await leerArchivoBase64(archivo);
      const rSubida = await llamar('subirDocumento', { nombreArchivo: archivo.name, tipoMime: archivo.type || 'application/pdf', contenidoBase64 });
      if (!rSubida.ok) { alert(rSubida.error); return; }
      documentoUrl = rSubida.url;
    ...
    if (!documentoUrl) { alert('Sube un archivo o pega un enlace'); return; }

    const r = await llamar('agregarDocumento', { id: escritoId, documentoUrl });
    if (!r.ok) { alert(r.error); return; }
