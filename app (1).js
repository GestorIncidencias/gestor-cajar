...
function renderTurnos() {
  ...
  if (!TURNOS_CACHE.length) { cont.innerHTML = '<p style="color:#9ca3af;font-size:13px;">Sin turnos este mes.</p>'; return; }
  cont.innerHTML = TURNOS_CACHE.map(t => {
    const sinAsignar = !t.agente;
    const esMio = t.agente === SESION.usuario;
    let botones = '';
    if (SESION.rol === 'admin') {
      botones = `<button class="pequena secundaria" onclick='abrirModalAsignarTurno(${JSON.stringify(t.id)}, ${JSON.stringify(t.fecha)}, ${JSON.stringify(t.horaInicio)}, ${JSON.stringify(t.horaFin)}, ${JSON.stringify(t.agente)})'>${sinAsignar ? 'Asignar' : 'Cambiar'}</button>`;
    } else if (esMio) {
      botones = `<button class="pequena secundaria" onclick="abrirModalCambio('${t.fecha}')">Cambiar</button>`;
    }
    return `
      <div class="fila-turno">
        <span class="turno-fecha">${formatearFechaCorta(t.fecha)}</span>
        <span class="turno-horario">${t.horaInicio}–${t.horaFin}</span>
        <span class="turno-agente">${sinAsignar ? '<span class="etiqueta etq-sin-asignar">Sin asignar</span>' : nombreAgente(t.agente)}</span>
        ${botones}
      </div>`;
  }).join('');
}
