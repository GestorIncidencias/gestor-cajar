...
function renderResumenIndispAdmin() {
  ...
  document.getElementById('resumenIndispAdmin').innerHTML = Object.entries(porAgente)
    .map(([ag, dias]) => `<p style="font-size:13px;"><b>${ag} · ${nombreAgente(ag)}</b>: ${dias.sort().join(', ')}</p>`).join('');
}
...
function bloqueTurno(t) {
  ...
  const chip = `<span class="turno-chip ${claseAdicional}" ${color ? `style="background:${color}"` : ''} ${clic ? `onclick="event.stopPropagation(); ${clic}"` : ''}>
