...
async function cargarTurnos() {
  ...
  const rTurnos = await llamar('getTurnos', { mes: mesISO(mesActual) });
  const rCambios = await llamar('getCambiosPendientes');
  const rIntereses = await llamar('getIntereses', { mes: mesISO(mesActual) });
  const rIndisp = SESION.rol === 'admin' ? await llamar('getIndisponibilidades', { mes: mesISO(mesActual) }) : null;
  if (rTurnos.ok) TURNOS_CACHE = rTurnos.turnos;
  if (rCambios.ok) CAMBIOS_CACHE = rCambios.cambios;
  if (rIntereses.ok) INTERESES_CACHE = rIntereses.porTurno;
  if (rIndisp && rIndisp.ok) {
    INDISP_CACHE = {};
    rIndisp.todas.forEach(t => { (INDISP_CACHE[t.fecha] = INDISP_CACHE[t.fecha] || []).push(t.agente); });
  }
  renderTurnos();
...
function renderTurnos() {
  ...
  for (let d = 1; d <= diasEnMes; d++) {
    ...
    const numDia = SESION.rol === 'admin'
      ? `<div class="cal-num" style="cursor:pointer;text-decoration:underline dotted;" onclick="abrirModalDia('${fecha}')" title="Gestionar turnos de este día">${d}</div>`
...
function bloqueTurno(t) {
  ...
  let chips;
  if (!lista.length) {
    chips = `<span class="turno-chip sin-asignar" ${clicAdmin ? `onclick="${clicAdmin}"` : ''}>Sin asignar<span class="chip-hora">${t.horaInicio}-${t.horaFin}</span></span>`;
  } else {
    chips = lista.map(usuario => {
      const clic = clicAdmin || (usuario === SESION.usuario ? `abrirModalCambio('${t.fecha}')` : '');
      return `<span class="turno-chip" style="background:${colorAgente(usuario)}" ${clic ? `onclick="${clic}"` : ''}>${usuario}<span class="chip-hora">${t.horaInicio}-${t.horaFin}</span></span>`;
    }).join('');
  }
...
async function cargarDisponibilidad() {
  document.getElementById('mesLabel').textContent = nombreMes(mesActual);
  const rTurnos = await llamar('getTurnos', { mes: mesISO(mesActual) });
  const rIndisp = await llamar('getIndisponibilidades', { mes: mesISO(mesActual) });
  if (!rTurnos.ok || !rIndisp.ok) return;
  renderCalendarioDisponibilidad(rTurnos.turnos, rIndisp.propias);

  const cardAdmin = document.getElementById('cardIndispAdmin');
  if (SESION.rol === 'admin') {
    cardAdmin.classList.remove('oculto');
    const porAgente = {};
    rIndisp.todas.forEach(t => { (porAgente[t.agente] = porAgente[t.agente] || []).push(formatearFechaCorta(t.fecha)); });
