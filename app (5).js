...
function renderEscritos() {
  ...
  const orden = document.getElementById('ordenEscritos') ? document.getElementById('ordenEscritos').value : 'antiguos';
  lista = [...lista].sort((a, b) => orden === 'recientes' ? (a.diasAbierto - b.diasAbierto) : (b.diasAbierto - a.diasAbierto));
  ...
  cont.innerHTML = lista.map(e => {
    ...
    const origen = e.origen || 'gestiona';
    return `
      <div class="card ${claseTarjeta}">
        <div class="fila">
          <h3>${e.numRegistro} — ${e.tipo}</h3>
          <span>
            <span class="etiqueta etq-origen-${origen}">${textoOrigen(origen)}</span>
        ...
        ${origen === 'gestiona'
          ...
          : e.detalleOrigen
            ? `<p style="font-size:12px;color:#374151;margin:2px 0;">${origen === 'organismo_externo' ? 'Organismo' : 'Solicitado por'}: ${e.detalleOrigen}</p>`
            : ''}
...
function abrirModalEscrito() {
  ...
  ['escNumRegistro', 'escSolicitante', 'escDni', 'escContacto', 'escDescripcion', 'escDocumentoUrl', 'escDetalleOrigen'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('escArchivo').value = '';
  document.getElementById('escFecha').value = new Date().toISOString().slice(0, 10);
  document.getElementById('escOrigen').value = 'gestiona';
  actualizarCamposOrigen();
  document.getElementById('modalEscrito').classList.remove('oculto');
}

function actualizarCamposOrigen() {
  const origen = document.getElementById('escOrigen').value;
  const esGestiona = origen === 'gestiona';
  document.getElementById('grupoNumRegistro').classList.toggle('oculto', !esGestiona);
  document.getElementById('grupoSolicitante').classList.toggle('oculto', !esGestiona);

  const necesitaDetalle = origen === 'concejalia_alcaldia' || origen === 'organismo_externo';
  document.getElementById('grupoDetalleOrigen').classList.toggle('oculto', !necesitaDetalle);
  document.getElementById('labelDetalleOrigen').textContent =
    origen === 'organismo_externo' ? 'Organismo' : 'Quién lo pidió';
}
...
async function guardarEscrito() {
  ...
  await conProcesando(boton, 'Guardando…', async () => {
    ...
    errorEl.textContent = '';
    if (origen === 'gestiona' && !numRegistro) { errorEl.textContent = 'El nº de registro es obligatorio'; return; }
    ...
    const r = await llamar('altaEscrito', {
      origen,
      numRegistro,
      detalleOrigen: document.getElementById('escDetalleOrigen').value.trim(),
