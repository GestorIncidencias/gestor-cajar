...
async function hacerLoginConCodigo() {
  ...
  await conProcesando(boton, 'Entrando…', async () => {
    ...
    if (r.debeCambiarPassword) {
      document.getElementById('cambiarPasswordTexto').textContent = 'Has entrado con un código de un solo uso. Ponte una contraseña nueva para tu usuario.';
      document.getElementById('cancelarPasswordPropiaBtn').classList.add('oculto');
      document.getElementById('nuevaPasswordPropia').value = '';
      document.getElementById('cambiarPasswordError').textContent = '';
      document.getElementById('modalCambiarPassword').classList.remove('oculto');
    }
  })();
}

function abrirModalCambiarPasswordVoluntario() {
  document.getElementById('cambiarPasswordTexto').textContent = 'Escribe la contraseña que quieras usar a partir de ahora.';
  document.getElementById('cancelarPasswordPropiaBtn').classList.remove('oculto');
  document.getElementById('nuevaPasswordPropia').value = '';
  document.getElementById('cambiarPasswordError').textContent = '';
  document.getElementById('modalCambiarPassword').classList.remove('oculto');
}
...
async function abrirModalPermuta(boton) {
  await conProcesando(boton, 'Cargando…', async () => {
...
// Por defecto se mantiene el orden de siempre (más antiguos primero, para no
// perder de vista lo que lleva más tiempo esperando). El botón permite
// invertirlo para ver antes lo recién dado de alta.
let ordenEscritosAntiguoPrimero = true;
function alternarOrdenEscritos() {
  ordenEscritosAntiguoPrimero = !ordenEscritosAntiguoPrimero;
  const boton = document.getElementById('ordenEscritosBtn');
  boton.textContent = ordenEscritosAntiguoPrimero ? '↑ Más antiguos primero' : '↓ Más recientes primero';
  renderEscritos();
}
...
function renderEscritos() {
  ...
  lista = [...lista].sort((a, b) => ordenEscritosAntiguoPrimero ? (b.diasAbierto - a.diasAbierto) : (a.diasAbierto - b.diasAbierto));
    ...
    return `
      <div class="card ${claseTarjeta}">
        ...
        <p style="font-size:12px;color:#374151;margin:2px 0;">
          ...
          ${e.documentos && e.documentos.length ? ' · ' + e.documentos.map((d, i) => `<a href="${d}" target="_blank">documento${e.documentos.length > 1 ? ' ' + (i + 1) : ''}</a>`).join(' · ') : ''}
        </p>
        ...
        ${e.estado === 'Hecho' && e.agentesFirmantes && e.agentesFirmantes.length
          ...
          : e.estado === 'Hecho' && SESION.rol === 'admin'
            ...
            : e.agentesAsignados && e.agentesAsignados.length ? `<p style="font-size:12px;color:#374151;">Asignado a: ${e.agentesAsignados.map(nombreAgente).join(' y ')}</p>` : ''}
        ...
        <div class="fila" style="margin-top:8px;">
          ...
          <button class="pequena secundaria" onclick="abrirModalAgregarDocumento('${e.id}')">Añadir documento</button>
          <span>
            ${e.estado === 'Pendiente' ? `<button class="pequena accion" onclick="reclamar('${e.id}')">Reclamar</button>` : ''}
            ${e.estado === 'Pendiente' && SESION.rol === 'admin' ? `<button class="pequena secundaria" onclick="abrirModalAsignarEscrito('${e.id}')">Asignar</button>` : ''}
            ${puedeLiberar(e) ? `<button class="pequena secundaria" onclick="liberar('${e.id}')">Liberar</button>` : ''}
            ${e.estado === 'En curso' && e.agentesAsignados && e.agentesAsignados.includes(SESION.usuario) ? `<button class="pequena accion" onclick="abrirModalCerrar('${e.id}')">Marcar hecho</button>` : ''}
            ${e.estado !== 'Hecho' && SESION.rol === 'admin' ? `<button class="pequena secundaria" onclick="abrirModalAdminCerrar('${e.id}')">Marcar hecho (admin)</button>` : ''}
          </span>
        </div>
      </div>`;
  }).join('');
...
function puedeLiberar(e) {
  ...
  if (e.agentesAsignados && e.agentesAsignados.includes(SESION.usuario)) return true;
...
// Mes propio de esta pantalla — deliberadamente independiente de mesActual
// (el de Cuadrante), para poder repasar meses anteriores de Carga de trabajo
// sin tener que ir a cambiarlo primero en Turnos.
let mesFirmas = new Date();
...
function cambiarPeriodoFirmas(periodo) {
  ...
  document.getElementById('mesSelectorFirmas').classList.toggle('oculto', periodo !== 'mes');
  cargarFirmas();
}

function cambiarMesFirmas(delta) {
  mesFirmas.setMonth(mesFirmas.getMonth() + delta);
  cargarFirmas();
}

async function cargarFirmas() {
  document.getElementById('mesLabelFirmas').textContent = nombreMes(mesFirmas);
  const params = periodoFirmas === 'mes' ? { periodo: 'mes', mes: mesISO(mesFirmas) } : { periodo: 'total' };
...
function abrirModalAsignarEscrito(id) {
  document.getElementById('asignarEscritoId').value = id;
  document.getElementById('listaAsignarOpciones').innerHTML = AGENTES.filter(a => a.activo).map(a =>
    `<label style="display:block;font-size:13px;margin:4px 0;">
      <input type="checkbox" value="${a.usuario}"> ${a.nombre}
    </label>`
  ).join('');
  document.getElementById('modalAsignarEscrito').classList.remove('oculto');
}

async function confirmarAsignarEscrito() {
  const boton = document.getElementById('confirmarAsignarBtn');
  await conProcesando(boton, 'Asignando…', async () => {
    const id = document.getElementById('asignarEscritoId').value;
    const agentes = Array.from(document.querySelectorAll('#listaAsignarOpciones input:checked')).map(i => i.value);
    if (!agentes.length) { alert('Selecciona al menos un agente'); return; }
    const r = await llamar('asignarEscrito', { id, agentes });
    if (!r.ok) { alert(r.error); return; }
    cerrarModal('modalAsignarEscrito');
    cargarEscritos();
  })();
}

function abrirModalAdminCerrar(id) {
  document.getElementById('adminCerrarId').value = id;
  document.getElementById('adminCerrarInformeUrl').value = '';
  document.getElementById('adminCerrarInformeArchivo').value = '';
  document.getElementById('listaAdminFirmantesOpciones').innerHTML = AGENTES.filter(a => a.activo).map(a =>
    `<label style="display:block;font-size:13px;margin:4px 0;">
      <input type="checkbox" value="${a.usuario}"> ${a.nombre}
    </label>`
  ).join('');
  document.getElementById('modalAdminCerrar').classList.remove('oculto');
}

async function confirmarAdminCierre() {
  const boton = document.getElementById('confirmarAdminCierreBtn');
  await conProcesando(boton, 'Guardando…', async () => {
    const id = document.getElementById('adminCerrarId').value;
    const agentesFirmantes = Array.from(document.querySelectorAll('#listaAdminFirmantesOpciones input:checked')).map(i => i.value);
    if (!agentesFirmantes.length) { alert('Selecciona al menos un agente'); return; }

    let informeUrl = document.getElementById('adminCerrarInformeUrl').value;
    const archivo = document.getElementById('adminCerrarInformeArchivo').files[0];
    if (archivo) {
    ...
    const r = await llamar('adminMarcarHecho', { id, agentesFirmantes, informeUrl });
    if (!r.ok) { alert(r.error); return; }
    cerrarModal('modalAdminCerrar');
    cargarEscritos();
  })();
}
...
// Sube varios archivos UNO A UNO (nunca en paralelo: Apps Script falla con
// llamadas concurrentes, ver manual técnico). onProgreso(i, total) es opcional,
// para actualizar el texto del botón mientras se suben.
async function subirVariosArchivos(fileList, onProgreso) {
  const urls = [];
  const archivos = Array.from(fileList || []);
  for (let i = 0; i < archivos.length; i++) {
    if (onProgreso) onProgreso(i + 1, archivos.length);
    const archivo = archivos[i];
    const contenidoBase64 = await leerArchivoBase64(archivo);
    const rSubida = await llamar('subirDocumento', { nombreArchivo: archivo.name, tipoMime: archivo.type || 'application/pdf', contenidoBase64 });
    if (!rSubida.ok) throw new Error(rSubida.error);
    urls.push(rSubida.url);
  }
  return urls;
}
...
async function guardarEscrito() {
  ...
  await conProcesando(boton, 'Guardando…', async () => {
    ...
    const documentoUrls = [];
    const urlPegada = document.getElementById('escDocumentoUrl').value.trim();
    if (urlPegada) documentoUrls.push(urlPegada);
    const archivos = document.getElementById('escArchivo').files;
    if (archivos.length) {
      try {
        const subidas = await subirVariosArchivos(archivos, (i, total) => {
          boton.innerHTML = `<span class="spinner"></span>Subiendo ${i}/${total}…`;
        });
        documentoUrls.push(...subidas);
...
function abrirModalAgregarDocumento(escritoId) {
  document.getElementById('agregarDocEscritoId').value = escritoId;
  document.getElementById('agregarDocUrl').value = '';
  document.getElementById('agregarDocArchivo').value = '';
  document.getElementById('modalAgregarDocumento').classList.remove('oculto');
}

async function guardarDocumentoAgregado() {
  const boton = document.getElementById('guardarDocumentoBtn');
  await conProcesando(boton, 'Guardando…', async () => {
    const escritoId = document.getElementById('agregarDocEscritoId').value;
    const documentoUrls = [];
    const urlPegada = document.getElementById('agregarDocUrl').value.trim();
    if (urlPegada) documentoUrls.push(urlPegada);
    const archivos = document.getElementById('agregarDocArchivo').files;
    if (archivos.length) {
      try {
        const subidas = await subirVariosArchivos(archivos, (i, total) => {
          boton.innerHTML = `<span class="spinner"></span>Subiendo ${i}/${total}…`;
    ...
    if (!documentoUrls.length) { alert('Sube al menos un archivo o pega un enlace'); return; }

    const r = await llamar('agregarDocumento', { id: escritoId, documentoUrls });
    if (!r.ok) { alert(r.error); return; }
    cerrarModal('modalAgregarDocumento');
    cargarEscritos();
  })();
}
...
async function cargarAvisos() {
  ...
  cont.innerHTML = r.avisos.map(a => `
    <div class="card card-aviso ${a.caducado ? 'aviso-caducado' : ''}">
      <div class="fila">
        <h3>${a.titulo}</h3>
      ...
      <p class="aviso-texto" style="white-space:pre-wrap;margin:6px 0;">${a.texto}</p>
