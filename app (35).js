    ...
    .map(([agente, v]) => {
      ...
      return `
        <div class="barra-equidad">
          ...
          <div class="barra-fondo" title="L-J y V mañana: ${v.entreSemana.toFixed(1)}h · V tarde-S-D: ${v.finDeSemana.toFixed(1)}h">
            <div class="barra-ls" style="width:${pctLS}%"></div>
            <div class="barra-fs" style="width:${pctFS}%"></div>
          </div>
          <div class="detalle-equidad">
            <span style="color:var(--azul-claro);">● ${v.entreSemana.toFixed(1)}h</span>
            <span style="color:var(--ambar);">● ${v.finDeSemana.toFixed(1)}h</span>
          </div>
        </div>`;
    }).join('');
}
